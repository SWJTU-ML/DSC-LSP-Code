...
def get_gaussian_kernel_my(D: torch.Tensor, scale, Ids: np.ndarray, device: torch.device, is_local: bool = True) -> torch.Tensor:   
    ...
    if Ids is not None:
        n, k = Ids.shape
        mask = torch.zeros([n, W.size(1)]).to(device=device)
        for i in range(len(Ids)):
            mask[i, Ids[i]] = 1
        W = W * mask
    return W
...
def Get_Center(x,rho,distances, indices):  
    ...
    for i in np.unique(pre_center):
        if len(np.where(pre_center==i)[0])<2:
            ...
            if temp[0] == i:
                ...
                lead = temp[0]
            pre_center[i]=pre_center[lead]
            pre_arrow[i]=pre_arrow[lead]  
            
    return pre_center, pre_arrow,pre_list  

def SNN(t, pre_center, rho, distances, indices):
    nois=set()
    cluster_nbr=[]
    for i in t:
        a=set()
